...
function makeDb(root: string): string {
  ...
  db.prepare("INSERT INTO normal_words (word) VALUES (?), (?), (?)").run("N1", "N2", "N3");
  const w = db.prepare("SELECT id, word FROM words").all() as { id: number; word: string }[];
  const n = db.prepare("SELECT id, word FROM normal_words").all() as { id: number; word: string }[];
  const wid = Object.fromEntries(w.map((x) => [x.word, x.id])) as Record<string, number>;
  const nid = Object.fromEntries(n.map((x) => [x.word, x.id])) as Record<string, number>;
  db.prepare("INSERT INTO word_to_normal_link (word_id, normal_id, weight) VALUES (?, ?, 1)").run(wid.alpha, nid.N1);
  db.prepare("INSERT INTO word_to_normal_link (word_id, normal_id, weight) VALUES (?, ?, 1)").run(wid.beta, nid.N2);
  db.prepare("INSERT INTO sentences (id, sentence, weight, duration) VALUES (101, 's101', 5, 9), (102, 's102', 2, 3), (103, 's103', 7, 1)").run();
  // sentence 101: links to N1,N2,N3
  db.prepare("INSERT INTO sentence_to_normal_link (normal_id, sentence_id, weight) VALUES (?, 101, 2)").run(nid.N1);
  db.prepare("INSERT INTO sentence_to_normal_link (normal_id, sentence_id, weight) VALUES (?, 101, 1)").run(nid.N2);
  db.prepare("INSERT INTO sentence_to_normal_link (normal_id, sentence_id, weight) VALUES (?, 101, 7)").run(nid.N3);
  // sentence 102: links to N1,N3
  db.prepare("INSERT INTO sentence_to_normal_link (normal_id, sentence_id, weight) VALUES (?, 102, 1)").run(nid.N1);
  db.prepare("INSERT INTO sentence_to_normal_link (normal_id, sentence_id, weight) VALUES (?, 102, 4)").run(nid.N3);
  // sentence 103: conflict case
  db.prepare("INSERT INTO sentence_to_normal_link (normal_id, sentence_id, weight) VALUES (?, 103, 3)").run(nid.N1);
  db.close();
  return dbPath;
}
...
describe("applyResultLinkFeedback", () => {
  it(
    "updates only matched normal_ids, applies +/- and deletes <=0, skip conflicts",
    { timeout: 15000 },
    () => {
    ...
    try {
      ...
      const rows = db
        .prepare("SELECT sentence_id, normal_id, weight FROM sentence_to_normal_link ORDER BY sentence_id, normal_id")
        .all() as { sentence_id: number; normal_id: number; weight: number }[];
      db.close();
      ...
      const s101 = rows.filter((r) => r.sentence_id === 101).map((r) => r.weight);
      expect(s101).toEqual([3, 2, 7]);
      // sentence 102: N1 deleted, N3 unchanged 4
      const s102 = rows.filter((r) => r.sentence_id === 102).map((r) => r.weight);
      expect(s102).toEqual([4]);
      // sentence 103 untouched due to conflict
      const s103 = rows.filter((r) => r.sentence_id === 103).map((r) => r.weight);
  ...
  it("inserts missing sentence_to_normal_link for high sentences vs story normals", () => {
    ...
    try {
      ...
      db.prepare("INSERT INTO normal_words (id, word) VALUES (10, 'N10')").run();
      db.prepare("INSERT INTO word_to_normal_link (word_id, normal_id, weight) VALUES (1, 10, 5)").run();
      db.prepare("INSERT INTO sentences (id, sentence, weight, duration) VALUES (201, 's', 1, 1)").run();
      db.close();
      ...
      const row = ro
        .prepare("SELECT weight FROM sentence_to_normal_link WHERE sentence_id = 201 AND normal_id = 10")
