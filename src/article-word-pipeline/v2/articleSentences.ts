...
export function sentenceLooksLikeVerbatimTechnical(s: string): boolean {
  ...
  if (!stripped.includes("\n")) {
    if (stripped.length > 120 && /^[\x00-\x7f\s]+$/.test(stripped)) {
  ...
  if (letters >= 20 && sym >= 12 && sym / Math.max(stripped.length, 1) >= 0.06) {
    return true;
  }
  return false;
}

function postProcessClamp(data: ArticleMemorySentencesData): ArticleMemorySentencesData {
  const out = [];
...
async function articleMemorySentencesLlm(
  ...
  const messagesJson = [
    ...
    { role: "user" as const, content: userBody + JSON_MODE_USER_SUFFIX_ARTICLE_SENTENCES },
  ];
