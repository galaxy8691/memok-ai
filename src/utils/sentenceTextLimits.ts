...
export function isEnglishDominantText(s: string): boolean {
...
export function clampGistText(gist: string): string {
  const s = gist.trim();
  if (!s) {
    return s;
  }
...
export function gistLengthOk(text: string): boolean {
  const s = text.trim();
  if (!s) {
    return true;
  }
