...
export type PredreamDecayResult = {
  ...
  /** `is_short_term=1` 且 `duration<=0` 且 `weight>=7` 被设为长期（`is_short_term=0`）的条数 */
...
/**
 * predream：全表句子 `duration` 减 1；再对「短期且 duration 已耗尽」按句 `weight` 分流：
 * - `weight >= 7` → `is_short_term = 0`（转长期）
