...
function pickTopSentenceId(result: z.infer<typeof ResultJsonForTopSentenceSchema>): number {
  ...
  for (let i = 1; i < rows.length; i += 1) {
    const cur = rows[i];
    if (cur.score > best.score || (cur.score === best.score && cur.id < best.id)) {
      best = cur;
    }
  }
  return best.id;
...
export async function mergeOrphanSentencesIntoTopScored(
  ...
  const parsed = ResultJsonForTopSentenceSchema.parse({ relevance: raw?.relevance });
  ...
  try {
    ...
    if (orphans.length === 0) {
      return { topSentenceId, orphansFound: 0, mergedCount: 0, deletedCount: 0 };
    }
